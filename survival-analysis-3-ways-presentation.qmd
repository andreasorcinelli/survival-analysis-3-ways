...
-   [ ] Return to motivating slides with naive and km estimaes
...
-   [ ] Clean up dead and duplicated code
-   [ ] WTF is up with the naive estimates plot?
-   [ ] Plot naive estimates and KM curves on same plot.
-   [ ] Add ggsurvfit for making plots to refs
-   [ ] Decide: Do I want to plot hazards?
-   [ ] Decide: Do I want to show extracting predictions from a cox surv object? It seems a little weird that you mention this is more involved but don't show it.
...
Imagine you work for a communication service provider like Xfinity or Verizon. A big part of your business is going to be focused on understanding renewals.
...
#TODO: clean this up. I don't think I even need a bunch of this anymore. 
...
## We cannot simply remove censored obersevations

```{r calc_naive_monthly_churn}
#| include: false
monthly_stats <- telco_data_clean |> 
  count(tenure, name = "monthly_total") |> 
  left_join(telco_data_clean |> 
              count(tenure, event), by = "tenure") |> 
  # add a cumulative sum of event 
  mutate(monthly_percent = n/monthly_total, 
         overall_total = dim(telco_data_clean)[1], 
         overall_percent = n/overall_total) |> 
  select(tenure, event, n, monthly_total, monthly_percent, overall_total, overall_percent)
```

```{r plot_naive_monthly_churn}
#| echo: false
monthly_stats |> 
  filter(event == 1, tenure != 0) |>
  ggplot(aes(x = tenure, y = monthly_percent)) +
...
```{r calc_naive_monthly_churn_2}
#| include: false
naive_estimate <- monthly_stats |> 
  filter(event == 1) |> 
  mutate(running_total = cumsum(monthly_total), 
         overall_percent = running_total/overall_total)
...
## Do the survival probabilities differ depending on the type of internet service? {.smaller}
...
## Does the hazard of churning differ depending on the type of internet service? {.smaller}

##### Old School: Survival Package

```{r}
# Alternative title for this slide: How can we quantify the difference between groups? {.smaller}
...
We can estimate the survival probability the number of customers who have renewed excluding censoring at that time, divided by the number of customers who had renewed just prior to that time.
...
|---------------------------|-------------------|--------------------------|
| Just a few lines of code; quick and easy to get started | A little more verbose                                                            | Dear god, don't ever use this in production code |
| Extracting predictions is a little more of a headache   | Consistency in how you specify fit, how you predict, and the object you get back | Helpful for giving an intuition                  |

## Don't simply exclude censored observations

TODO: Go back to plot comparing naive estimates vs. those from Kaplan Meier. Put both curves on the same plot.

I hope I have convinced you of . . . . There are options for implementing survival analysis in R.
...
```{r}
naive_estimate_lung <- lung |> 
  count(time, name = "daily_total") |> 
  left_join(lung |> 
              count(time, status), by = "time") |> 
  #filter(status == 1) |> 
  mutate(running_total = sum(n), 
         percent_naive = running_total/total) |> 
  arrange(time)
...
```{r}
# cbind(lung |> 
#   count(time, status, name = "daily_total"), nrow(lung))
